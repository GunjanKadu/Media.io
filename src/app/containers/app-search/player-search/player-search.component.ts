...
@Component({
  ...
  template: `
    <form class="navbar-form form-search is-flex-row" [formGroup]="searchForm">
      <input
        placeholder="Find My Media..."
        ngxTypeahead
      ...
      />
      <button
        class="btn btn-transparent btn-submit is-flex-row is-flex-valign"
        title="search with echoes"
      >
    ...
    <button class="btn btn-filter btn-transparent is-flex-row is-flex-valign">
      <icon
        name="filter"
        [ngClass]="{
          'text-primary': filtersForm.value.duration || filtersForm.value.hd
        }"
      ></icon>
      <form class="filters is-rounded" [formGroup]="filtersForm">
        ...
        <icon
          name="trash"
          *ngIf="filtersForm.value.duration || filtersForm.value.hd"
          (click)="clearFilters()"
        ></icon>
      </form>
    </button>
